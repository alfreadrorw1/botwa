/*
# Base By Fauzialifata
# Pengembang : Zion Dev/Kyzo Yamada
# Devoloper : Zion
# Baileys By Zion
...
if (command === 'register' || command === 'reg') {
  ...
  if (!global.register) 
    return reply("⚠️ Fitur register sedang dimatikan oleh owner!");
  ...
  if (isNaN(age)) return reply('❌ Umur harus berupa angka!');
  if (age < 10 || age > 70) 
    return reply('⚠️ Umur harus antara 10–70 tahun!');
...
// ==========================
// 🗑️ UNREGISTER (PAKAI SN)
// ==========================
if (command === 'unregister' || command === 'unreg') {

  if (!global.register) 
    return reply("⚠️ Fitur register sedang dimatikan!");
  ...
  if (text.trim() !== user.sn) 
    return reply('❌ *Kode SN salah!*');
  ...
  reply('🗑️ *Akun kamu berhasil dihapus dari database.*');
  return;
}
...
const menuV1 = (teks = '') => {
  const latensi = performance.now().toFixed(3)

  const limits = checkLimit(sender)
  const opr = cekOperator(senderNumber)

const caseDir = './Hoshino';
...
╭━━〔 🤖 𝗦𝗠𝗔𝗥𝗧 𝗔𝗦𝗦𝗜𝗦𝗧𝗔𝗡𝗧 〕━━╮
┃ ${namaBot}
┃ _Always ready, always smart_
╰━━━━━━━━━━━━━━━━━━━━━━╯

👤 𝗨𝗦𝗘𝗥 𝗣𝗥𝗢𝗙𝗜𝗟𝗘
├─ Name     : ${pushname}
├─ Number   : ${senderNumber}
├─ Network  : ${opr.operator}
├─ Access   : ${isOwner ? '👑 Owner' : isPremium ? '⭐ Premium' : '🆓 Free'}
└─ Limit    : ${limits.total}
...
🧠 _Navigate features using the smart menu below_
${teks}
`
  ...
  sock.sendMessage(m.chat, {
    image: { url: thumbnail },
    caption: smartMenu,
    footer: `⚡ Powered by ${namaBot} • v${version}`,
    viewOnce: true,
    buttons: [
      {
        buttonId: '.owner',
            ...
            sections: [
              {
                title: '🚀 QUICK ACCESS',
                rows: [
                  { title: '📜 All Features', description: 'Show all available commands', id: '.allmenu' },
                  { title: '📂 Category List', description: 'Browse by feature category', id: '.menulist' }
                ]
              },
              {
                title: '🧩 FEATURE CATEGORIES (31)',
                rows: [
                  { title: '👑 Owner', description: 'Bot control & configuration', id: '.menuowner' },
                  { title: '🧑‍🧒‍🧒 Group', description: 'Group management tools', id: '.menugroup' },
                  { title: '🤔 Other', description: 'Additional utilities', id: '.menuother' },
                  { title: '🤖 AI', description: 'AI chat & generators', id: '.menuai' },
                  { title: '📥 Download', description: 'Media downloader', id: '.menudownload' },
                  { title: '🔁 Convert', description: 'Media converter', id: '.menuconvert' },
                  { title: '🌥️ Cpanel', description: 'Server panel tools', id: '.menucpanel' },
                  { title: '🌤️ Cpanel+', description: 'Advanced server panel', id: '.menucpanel2' },
                  { title: '🤣 Fun', description: 'Entertainment features', id: '.menufun' },
                  { title: '🇯🇵 Anime', description: 'Anime & manga tools', id: '.menuanime' },
                  { title: '🎮 Game', description: 'Interactive mini games', id: '.menugame' },
                  { title: '📍 Primbon', description: 'Primbon & predictions', id: '.menuprimbon' },
                  { title: '🎆 Maker', description: 'Sticker & logo maker', id: '.menumaker' },
                  { title: '⚔️ RPG', description: 'Leveling & battle system', id: '.menurpg' },
                  { title: '🖼️ Edit Image', description: 'Image enhancement', id: '.menueditimg' },
                  { title: '🌅 Ephoto', description: 'Text & photo effects', id: '.menuephoto' },
                  { title: '🛍️ Store', description: 'Shop & topup service', id: '.menustore' },
                  { title: '💪 Quotes', description: 'Motivation & quotes', id: '.menuquotes' },
                  { title: '💌 Menfess', description: 'Anonymous confession', id: '.menumenfess' },
                  { title: '🔀 Random', description: 'Random content', id: '.menurandom' },
                  { title: '🌐 News', description: 'Latest news updates', id: '.menuberita' },
                  { title: '🌥️ Linode', description: 'Linode cloud tools', id: '.menulinode' },
                  { title: '🔞 NSFW', description: 'Adult content (restricted)', id: '.menunsfw' },
                  { title: '🈺 Genshin', description: 'Genshin Impact tools', id: '.menugenshin' },
                  { title: '🔒 Encrypt', description: 'Encode & decode data', id: '.menuenc' },
                  { title: '🌤️ DigitalOcean', description: 'DO cloud management', id: '.menudigitalocean' },
                  { title: '🕌 Islamic', description: 'Islamic utilities', id: '.menuislam' },
                  { title: '🔍 Search', description: 'Advanced search', id: '.menusearch' },
                  { title: '☠️ Bug', description: 'Testing & debug tools', id: '.menubug' },
                  { title: '🧐 Stalk', description: 'Profile stalking tools', id: '.menustalk' },
                  { title: '⬇️ Install', description: 'Install packages & tools', id: '.menuinstall' },
                  { title: '⚒️ Tools', description: 'System & Bot Utility', id: '.menutools' }
                ]
              },
              {
                title: '📦 SYSTEM',
                rows: [
                  { title: '📁 Script Info', description: 'About this bot', id: '.sc' }
                ]
              }
            ]
          })
...
const menuV2 = (teks = '') => {
  const latency = performance.now().toFixed(3)
  const limits = checkLimit(sender)
  ...
  // OWNER HIDDEN INFO
  const ownerHidden = isOwner
    ? `
🔐 𝗢𝗪𝗡𝗘𝗥 𝗗𝗜𝗔𝗚𝗡𝗢𝗦𝗧𝗜𝗖
┃ Node ID    : ${Math.random().toString(36).slice(2, 10).toUpperCase()}
...
┃ _Precision • Security • Performance_
┗━━━━━━━━━━━━━━━━━━━━━━━━┛
...
┃ 📱 Number    : ${senderNumber}
┃ 🔐 Access    : ${accessLevel}
┃ 🎟️ Limit     : ${limits.total}
┗━━━━━━━━━━━━━━━━━━━━━━━━━┛

🧠 𝗦𝗬𝗦𝗧𝗘𝗠 𝗦𝗧𝗔𝗧𝗨𝗦
┃ ⏳ Uptime    : ${runtime(process.uptime())}
┃ 🌐 Mode      : ${sock.public ? 'PUBLIC NETWORK' : 'PRIVATE NODE'}
┃ 🧩 Modules   : ${totalFitur} Active
┃ 👥 Users     : ${Object.values(db.users).length}
┃ ⚡ Latency   : ${latency} ms
┗━━━━━━━━━━━━━━━━━━━━━━━━━━┛
${ownerHidden}

🚀 _System online. Awaiting command execution._
${teks}

⚡ Powered by ${namaBot} • v${version}
`
  ...
  sock.sendMessage(m.chat, {
    ...
    contextInfo: {
      mentionedJid: [sender],
      externalAdReply: {
        title: `${namaBot} • AI CORE SYSTEM`,
        body: `${accessLevel} • Build ${version}`,
        thumbnailUrl: thumbnail2,
        sourceUrl: linkgc,
        mediaType: 1,
        renderLargerThumbnail: true
...
┃ Precision • Stability • Intelligence
┗━━━━━━━━━━━━━━━━━━━━━━━┛
...
🧠 𝗦𝗬𝗦𝗧𝗘𝗠 𝗜𝗡𝗙𝗢
┃ ⏳ Runtime   : ${runtime(process.uptime())}
┃ 🌐 Mode      : ${sock.public ? 'PUBLIC MODE' : 'PRIVATE MODE'}
┃ 🧩 Features  : ${totalFitur} Active
┃ 👥 Users     : ${Object.values(db.users).length}
┃ ⚡ Speed     : ${latency} ms
┗━━━━━━━━━━━━━━━━━━━━━━━━┛
...
`
  ...
  sock.sendMessage(m.chat, {
    ...
    contextInfo: {
      ...
      externalAdReply: {
        title: `${namaBot} • ELITE SYSTEM`,
        body: `${access} • Build ${version}`,
        thumbnailUrl: thumbnail2,
        renderLargerThumbnail: true,
        mediaType: 1,
        previewType: 1,
...
sock.readMessages([m.key])
   ...
   const fitur1 = () =>{
            var mytext = fs.readFileSync("./Hoshino.js").toString()
...
async function Hoshino(promptName, text) {
        ...
        switch (intent.intent) {
            ...
            default:
                ...
                if (data?.result) {
                    ...
                    let ai = await Hoshino(pushname, text);
                    return reply(ai);
                }
        }
...
if (db.chats[m.chat].antispam) {
    ...
    if (fast && same) {
        ...
        if (user.spam >= 3) {
            ...
            await sock.sendMessage(m.chat, {
                text: `🔇 @${m.sender.split("@")[0]} di-mute selama *1 menit*\n\nAlasan: *Spam pesan*`,
                mentions: [m.sender]
            })
            ...
            reply(`⚠️ Warning spam! (${user.spam}/3)`)
        }
    }
...
async function waitEnhance(url, maxTry = 10) {
  for (let i = 0; i < maxTry; i++) {
    let { data } = await axios.get(
      'https://hoshino-apis.vercel.app/api/hd',
...
✨ Total kategori: 33
✨ Gunakan: .menu-<kategori>
✨ Contoh: .menu-download
`
...
//============= [ COMMANDS ] ====================================================
        ...
        const tks = `
🚀 SCRIPT BOT HOSHINO ASSISTEN V1.0 🚀
Solusi Bot WhatsApp all-in-one dengan fitur super lengkap, stabil, dan siap digunakan tanpa ribet.
🔥 KEUNGGULAN UTAMA

Hoshino Assisten V1.0 dikembangkan untuk kebutuhan bot modern: AI, otomatisasi, multimedia, dan keamanan data dalam satu script.
...
🔐 KEAMANAN DATA
📌 DATABASE SECURITY AKTIF
✔️ Data user aman
✔️ Proteksi database
✔️ Minim risiko kebocoran data

🎯 COCOK UNTUK
✅ Bot pribadi
✅ Bot grup
✅ Bot bisnis / toko online
✅ Bot AI assistant
✅ Bot multi-fungsi skala besar
⚙️ SIAP PAKAI
✔️ Install → Run → Langsung aktif
✔️ Tanpa konfigurasi ribet
✔️ Cocok untuk pemula & developer

🔖 DOWNLOAD
👉 https://whatsapp.com/channel/0029VbC75vOHQbS30masND0x

🔥 HOSHINO ASSISTEN V1.0
Bot cerdas • Fitur melimpah • Stabil • Aman
`
        sock.sendMessage(m.chat, {
    productMessage: {
        title: "Script Hoshino Assiten V1.0",
...
┃ ⌁ Terpakai  : *${formatp(os.totalmem() - os.freemem())}*
┃
┃ 📂 *STORAGE*
┃ ⌁ Total     : *${storage.totalGb} GB*
┃ ⌁ Digunakan : *${storage.usedGb} GB (${storage.usedPercentage}%)*
┃ ⌁ Tersisa   : *${storage.freeGb} GB (${storage.freePercentage}%)*
┃
┃ ⏳ *RUNTIME*
...
${Object.keys(cpu.times)
  .map(type => `┃ ⌁ ${type.padEnd(5)} : ${(100 * cpu.times[type] / cpu.total).toFixed(2)}%`)
  .join('\n')}
┃
╰━━━━━━━━━━━━━━━━━━━━╯
✨ *Bot berjalan normal & stabil*
`
...
✨ Total Fitur   : *${totalFitur}*
  ...
  sock.sendMessage(m.chat, { 
    pollResultMessage: { 
        ...
        pollVotes: [
            {
                optionName: "Total Case",
                optionVoteCount: totalCase
            },
            {
                optionName: "Total Plugin",
                optionVoteCount: totalPlugin
            },
            {
                optionName: "Total Fitur",
...
case "menutools": {
    try {
        ...
        if (menuVersion === 'v1') menuV1(menutools);
        else if (menuVersion === 'v2') menuV2(menutools);
        else if (menuVersion === 'v3') menuV3(menutools);
        else menuV1(menutools);
...
case 'kickall': {
 ...
 if (waktu < 500) return reply('⚠️ Minimal jeda 500 ms')

 let teks = `⚠️ *KONFIRMASI KICK SEMUA MEMBER*\n
• Jeda: ${waktu} ms
• Semua member akan dikeluarkan
• Owner & Bot tidak ikut
...
case 'kickall-konfirm': {
 ...
 reply(`🚀 Memulai kick ${members.length} member...\nJeda: ${waktu} ms`)
 ...
 reply('✅ Kick all selesai')
}
break
case 'cancel':
 reply('❌ Aksi dibatalkan')
break
case 'listgroup':
case 'grouplist':
case 'gclist':
case 'listgc': {
  ...
  try {
    ...
    if (!groups.length)
      return reply('❌ Bot belum gabung di grup manapun.')
...
case 'setimgmenu': {
  ...
  if (!text || !text.startsWith('http'))
    return reply('⚠️ Masukkan URL gambar!\nContoh:\n.setimgmenu https://example.com/img.jpg')

  let media = loadMedia()
  media.thumbnail2 = text
  saveMedia(media)

  reply('✅ Image menu berhasil diubah!')
}
break
...
case 'setthumbnailmenu': {
  if (!isOwner) return reply('❌ Khusus owner')
  if (!text || !text.startsWith('http'))
    return reply('⚠️ Masukkan URL thumbnail!\nContoh:\n.setthumbnailmenu https://example.com/thumb.jpg')

  let media = loadMedia()
  media.thumbnail = text
  saveMedia(media)

  reply('✅ Thumbnail menu berhasil diubah!')
}
break
...
case 'setvideomenu': {
  if (!isOwner) return reply('❌ Khusus owner')
  if (!text || !text.startsWith('http'))
    return reply('⚠️ Masukkan URL video!\nContoh:\n.setvideomenu https://example.com/video.mp4')

  let media = loadMedia()
  media.video = text
  saveMedia(media)

  reply('✅ Video menu berhasil diubah!')
}
break
...
case 'setaudiomenu': {
  if (!isOwner) return reply('❌ Khusus owner')
  if (!text || !text.startsWith('http'))
    return reply('⚠️ Masukkan URL audio!\nContoh:\n.setaudiomenu https://example.com/audio.mp3')

  let media = loadMedia()
  media.audio = text
  saveMedia(media)

  reply('✅ Audio menu berhasil diubah!')
}
break
case 'anticall':
case 'antical': {
  ...
  if (args[0] === 'on') {
    ...
    reply('✅ Anti Call *AKTIF*')
  } else if (args[0] === 'off') {
    global.antiCall = false
    reply('❌ Anti Call *NONAKTIF*')
  } else {
    reply(`Contoh:
.antical on
.antical off`)
...
if (!isOwner) return reply('❌ Khusus Owner')
  try {
    ...
    if (!q || !q.mimetype) {
      return reply('❌ *Reply file .zip atau .html* yang ingin diupload');
    }
    
    const mime = q.mimetype || '';
    if (!mime.includes('zip') && !mime.includes('html')) {
      return reply('❌ *Format file tidak didukung*\nHanya file .zip atau .html');
    }
    
    const webName = text.trim().toLowerCase().replace(/[^a-z0-9-_]/g, '');
    if (webName.length < 3) {
      return reply('❌ Nama web minimal 3 karakter');
    }
    ...
    if (!media) {
      return reply('❌ Gagal mengunduh file');
    }
    
    const filesToUpload = [];
    
    if (mime.includes('zip')) {
      await reply('📦 *Mengekstrak file ZIP...*');
      ...
      try {
        ...
        if (!hasIndex && !filesToUpload.some(f => 
          ...
          return reply('❌ File HTML tidak ditemukan dalam ZIP');
        }
        
      } catch (zipError) {
        return reply(`❌ Error ekstrak ZIP: ${zipError.message}`);
      }
    ...
    if (filesToUpload.length === 0) {
      return reply('❌ Tidak ada file yang valid untuk diupload');
    }
    
    await reply('🚀 *Mendeploy ke Vercel...*');
    ...
    if (!deployRes.ok) {
      ...
      return reply(`❌ Gagal deploy: ${result.error?.message || 'Unknown error'}`);
    }
    
    const url = `https://${webName}.vercel.app`;
    
    // Success
    await reply(`✅ *Website Berhasil Dibuat!*

📝 *Nama:* ${webName}
🌐 *URL:* ${url}
⏱️ *Status:* ${result.readyState || 'Deploying...'}

_Situs akan aktif dalam 1-2 menit._`);
    
  } catch (error) {
    console.error('Handler Error:', error);
    reply(`❌ Terjadi error: ${error.message}`);
  }
} break;
case 'resetdb':
case 'resetdatabase': {
...
case 'addsaldo': {
  ...
  if (!global.db.users[target]) return reply('❌ User belum ada di database!');

  jumlah = parseInt(jumlah);
  if (isNaN(jumlah) || jumlah <= 0) return reply('❌ Jumlah harus berupa angka dan lebih dari 0!');
...
• .addlimit 6281234567890 25`)
  ...
  if (isNaN(amount) || amount <= 0) {
    return reply('❌ Jumlah limit harus angka positif!')
  }
  ...
  if (target.startsWith('@')) {
    ...
    if (!mentioned) {
      return reply('❌ Tag user yang valid!')
    }
    ...
    return reply('❌ Format target tidak valid! Gunakan @user atau nomor 628xxx')
  }
...
try {
    if (!fs.existsSync('./database/sewa.json')) {
        return reply("📭 Tidak ada data sewa");
    }
...
async function saveSewaData(groupId, totalDetik) {
    ...
    // Simpan ke file
    fs.writeFileSync('./database/sewa.json', JSON.stringify(filteredData, null, 2));
}
case "addsewa": {
...
case 'antidelete': {
    ...
    if (!isAdmins && !isCreator) return m.reply('Admin only')

    if (args[0] === 'on') {
        global.antiDelete[m.chat] = true
        return m.reply('✅ Anti Delete AKTIF')
    }

    if (args[0] === 'off') {
        delete global.antiDelete[m.chat]
        return m.reply('❌ Anti Delete MATI')
    }
...
case 'warn': {
  ...
  if (args[0] === 'on') {
    ...
    reply('✅ Warn diaktifkan')
  } else if (args[0] === 'off') {
    db.chats[m.chat].warn = false
    reply('❌ Warn dimatikan')
  } else {
    reply('Contoh:\n.warn on\n.warn off')
  }
}
...
case 'delwarn': {
  ...
  await sock.sendMessage(m.chat, {
    text: `✅ Warn @${target.split('@')[0]} berhasil dihapus`,
    mentions: [target]
  }, { quoted: fakeQuoted })
}
break
case 'antispam': {
    ...
    if (args[0] === "on") {
        ...
        return reply("✔️ AntiSpam berhasil diaktifkan!");
    } else if (args[0] === "off") {
        db.chats[m.chat].antispam = false;
        return reply("❌ AntiSpam dimatikan!");
    } else {
...
case "ptv": {
    if (!text) {
        return reply(
            `⚠️ Contoh:\n` +
            `• *.${command} kucing*\n` +
            `• *.${command} kucing.162288@g.us* (owner)`
        );
    }
    ...
    if (text.includes('.')) {
        ...
        // jika pakai target → owner only
        if (!isOwner) {
            return reply("⛔ Fitur kirim ke ID hanya untuk OWNER!");
        }
    } else {
        query = text;
...
if (tujuan.endsWith('@newsletter') && !isOwner) {
    return reply(`❌ Hanya owner yang boleh mengirim ke saluran!`);
}
...
await sock.sendMessage(tujuan, {
  ...
  reply(`✅ Video pendek (bulat) berhasil dikirim!`);
}
break;
// Command limit
case "tomedia": {
...
const user = global.db.users[m.sender]
  ...
  if (user.lastclaim && (now - user.lastclaim) < cooldown) {
    ...
    return reply(`⏳ *Anda sudah claim hari ini!*\nTunggu ${hours} jam ${minutes} menit lagi.`)
  }
            ...
            viewOnceMessage: {
                message: {
                    ...
                    interactiveMessage: proto.Message.InteractiveMessage.create({
                        ...
                        header: proto.Message.InteractiveMessage.Header.create({
                            title: name || 'Hoshino-MD', // opsional
                            subtitle: '',
                            hasMediaAttachment: false
                        }),
                        nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.create({
 ...
 if (!text) return reply(`📌 Contoh:\n${cmd} Ini contoh teks yang akan diunggah ke Pastebin`)
  ...
  try {
    ...
    if (url.startsWith('Bad API request')) {
      return reply('❌ Gagal membuat Pastebin:\n' + url)
    }
    reply(`✅ *Berhasil membuat paste:*\n${url}`)
  } catch (e) {
    console.error(e)
    reply('⚠️ Gagal mengirim permintaan ke Pastebin.')
  }
}
break
case "getpp": {
...
case 'bratgura': {
    ...
    try {
        ...
        // 4️⃣ upload hasil gura
        const guraPublic = await CatBox(guraPath)

        // 5️⃣ KIRIM STICKER (INI KUNCI 🔑)
        await sock.sendImageAsSticker(
            m.chat,
            guraPublic,
            m,
...
case 'upscale': {
  if (!m.quoted && !m.message?.imageMessage) {
    return reply('❌ Reply / kirim gambar dengan caption *upscale*')
  }

  reply('⏳ Upscaling gambar, mohon tunggu...')
  ...
  try {
    ...
    reply('❌ Terjadi kesalahan saat upscale')
    console.error(err)
  }
}
break
...
case 'togif': {
  ...
  await sock.sendMessage(
    m.chat,
    {
      video: { url: mediaUrl }, // ⬅️ WAJIB url
      gifPlayback: true,
      caption: '✅ Sticker / Video berhasil jadi GIF'
    },
    { quoted: fakeQuoted }
  )
}
...
case 'toimg': {
    ...
    try {
            ...
            exec(
                ...
                async (err) => {
                    ...
                    if (!fs.existsSync(outGif))
                        return reply('❌ File GIF tidak ditemukan')
                    ...
                    await sock.sendMessage(
                        m.chat,
                        {
                            video: buffer,
                            gifPlayback: true,
                            caption: '✅ Sticker video berhasil diubah ke GIF'
                        },
                        { quoted: fakeQuoted }
                    )
            ...
            exec(
                ...
                async (err) => {
                    ...
                    if (err) {
                        ...
                        return reply('❌ Gagal mengkonversi sticker ke gambar')
                    }

                    if (!fs.existsSync(outPng))
                        return reply('❌ File image tidak ditemukan')
                    ...
                    await sock.sendMessage(
                        ...
                        {
                            image: buffer,
                            caption: '✅ Sticker foto berhasil diubah ke gambar'
                        },
                        { quoted: fakeQuoted }
                    )
        ...
        console.error(e)
        reply('❌ Terjadi kesalahan saat memproses sticker')
    }
}
break
case 'tovoicenote':
...
/**
 • Fitur: Overchat AI
 • Type: CJS (CASE)
 • BaseUrl: https://overchat.ai
 • Creator: convert by ChatGPT
*/
...
case 'aioverchat': {
  ...
  try {
    ...
    if (!result.trim()) return reply('❌ Tidak ada jawaban yang dihasilkan')
...
/**
  *» Nama :* — [ ZAHANAT AI ] —
  *» Type :* Plugin - CJS (CASE)
  *» Base Url :* https://chat.zahanat.ai
  *» Creator :* -Ɗαnčoᴡ々
**/
...
const userConversations = {}
  ...
  try {
    ...
    return reply('❌ Gagal menghubungi server Zahanat AI')
  }

  if (!res || !res.ok)
    return reply('❌ Server Zahanat AI menolak permintaan')

  let raw
  try {
    raw = await res.text()
  } catch {
    return reply('❌ Gagal membaca response dari server')
  }
...
case 'clearchat': {
  ...
  reply('✅ Conversation berhasil direset')
}
break
/*
- [ Cloude ]
...
case 'claude': {
  ...
  try {
    ...
    return reply('❌ Gagal menghubungi server Claude')
  }
...
const MODELS = [
  ...
  try {
    ...
    return reply('❌ Gagal menghubungi server Heck.ai')
  }
  ...
  try {
    ...
    return reply('❌ Error saat membaca stream')
  }
...
/**
  *» Nama :* — [ SANDBOX AI ] —
  *» Type :* Plugin - CJS (CASE)
  *» Base Url :* https://chatsandbox.com
  *» Creator :* -Ɗαnčoᴡ々
**/
...
const CHARACTERS = [
  ...
  try {
    ...
    reply('❌ Gagal menghubungi Sandbox AI')
  }
}
break
/**
  *» Nama :* — [ WRITECREAM AI ] —
  *» Type :* Plugin - CJS (CASE)
  *» Base Url :* https://www.writecream.com
  *» Creator :* -Ɗαnčoᴡ々
**/
case 'writecreamai': 
case 'aiwritecream': 
case 'writecream': {
  ...
  try {
    ...
    reply('❌ Gagal menghubungi WriteCream')
  }
}
break
case 'turbo':
...
case 'turbochat': {
  ...
  try {
    ...
    reply('❌ Gagal menghubungi TurboChat')
  }
}
break
case "ai": {
  try {
    ...
    let imagePath = null
    let prompt = text || "Jelaskan isi foto ini dengan singkat dan jelas."
...
break
  ...
  *» Nama :* — [ ASISTEN AI ] —
  *» Type :* Case - CJS
  *» Base Url :* https://chat.asistenai.com
...
break
			...
			case 'aicastorice': {
			if (!text) return reply("contoh : "+cmd+" Hai")
			const api = await fetchJson(`https://hoshino-apis.vercel.app/api/castorice?q=${text}`)
			...
			{ 
			  if (!text) return reply("contoh "+cmd+" hai")
			  try {
			  const api = await fetchJson(`https://theresapisv3.vercel.app/ai/wormgpt?text=${text}`)
			  reply(api.result)
			     } catch (e) {
        console.log(e);
        reply('Terjadi error');
...
case "elevenlabs": {
    ...
    const buttons = [
        { buttonId: ".el_bella", buttonText: { displayText: "🎙️ Bella" }, type: 1 },
        { buttonId: ".el_adam", buttonText: { displayText: "🗣️ Adam" }, type: 1 },
        { buttonId: ".el_miku", buttonText: { displayText: "🎧 Miku" }, type: 1 },
        { buttonId: ".el_jokowi", buttonText: { displayText: "🇮🇩 Jokowi" }, type: 1 },
        { buttonId: ".el_prabowo", buttonText: { displayText: "🦅 Prabowo" }, type: 1 }
    ]
...
case "el_prabowo": {
    ...
    try {
        reply("🎙️ Sedang mengubah teks menjadi suara...")
...
async function instagram(url) {
  ...
  try {
    if (!args[0]) {
      return reply(`❌ *Masukkan URL Instagram!*\n\n*Contoh:*\n${cmd} https://www.instagram.com/p/xxx\n\n*Support:*\n• Post (Image/Video)\n• Reels\n• Stories\n• Multiple Images`)
    }

    const url = args[0]

    if (!url.match(/instagram\.com|instagr\.am/i)) {
      return reply('❌ URL bukan dari Instagram!')
    }

    await reply('⏳ *Sedang mengunduh media dari Instagram...*\n_Mohon tunggu sebentar_')
    ...
    for (const [index, media] of result.media.entries()) {
      try {
        ...
        await reply(`❌ Gagal mengirim media ${media.index}`)
      }
    }
...
case 'mediafiredl': 
case 'mfdl': {
    if (!text) return reply(`❌ Masukkan link Mediafire!\n\n📌 Contoh:\n${prefix + command} https://www.mediafire.com/file/xxxxxx`)
    if (!isUrl(text) || !text.includes('mediafire.com')) return reply('❌ Link tidak valid! Pastikan itu link Mediafire.')

    reply('⏳ Sedang memproses link Mediafire...')

    try {
        const data = await mediafire(text)
        if (!data || !data.downloadUrl) return reply('❌ Gagal mengambil data dari Mediafire.')
        ...
        if (data.downloadUrl && data.fileSize && !data.fileSize.includes('GB')) {
            ...
            reply('⚠️ File terlalu besar untuk dikirim langsung.\nSilakan unduh manual melalui link di atas.')
        }

    } catch (e) {
        console.error(e)
        reply('❌ Terjadi kesalahan saat memproses link Mediafire.')
    }
}
break
case 'douyin':
...
case "ttsearch":
case "tts": {
      if (!text) return reply(`⚠️ Eits, kakak lupa kasih kata kunci! 😗 Coba ketik kayak gini ya: *.${command} jj epep* biar Aku bisa bantu cari yang kakak mau! 🔍💬`);
      try {
...
case 'tembak':
            case 'jadian': {
              ...
              if (user === m.sender) return reply("Halah mawunya jadian sama diri sendiri😢")
              ...
              if (pasangan2 === user) {
                reply(`Kamu kan udah jadian smaa dia 😤`)
              } else if (pasangan) {
            ...
            case 'terima': {
              ...
              if (sock.jadian[m.sender]) {
                ...
                reply("ihh kaga ada yang nembak kamu 🙄")
              }
            }
            break
            ...
            case 'tolak': {
              ...
              if (sock.jadian[m.sender]) {
                ...
                reply("Dihh lu nolak siapa, setan ? 🙄")
              }
            }
            break
            ...
            case 'putus': {
              ...
              if (pasangan) {
                ...
                reply("Kamu jomblo ngapain putus njir 🙄")
              }
            }
            break
...
case 'tekateki': {
  if (!isGroup) return reply(mess.group)
  if (global.tekateki) return reply('⚠️ Masih ada game yang berlangsung!')
...
case 'tebaktebakan': {
  if (!isGroup) return reply(mess.group)
  if (global.tebaktebakan) return reply('⚠️ Masih ada game yang berlangsung!')
...
case "carbonify": {
  try {
    ...
    reply('❌ Terjadi kesalahan saat membuat carbonify')
  }
}
break
...
case "animegirl": {
  try {
    ...
    console.error(err);
    reply("❌ Gagal mengambil gambar animegirl.");
  }
}
break;         
case "attp": {
...
case "fakestory": {
  try {
    ...
    if (!/image/.test(mime))
      return reply("❌ Reply foto yang akan dijadikan PP fake story");
    ...
    console.error(err);
    reply("❌ Error membuat Fake Story.");
  }
}
break;
case "faketweet": {
  try {
    ...
    if (!/image/.test(mime))
      return reply("❌ Reply foto untuk dijadikan PP Twitter");
    ...
    let ppUrl = upload.data.trim();
    if (!ppUrl.startsWith("http"))
      return reply("❌ Gagal upload PP ke CatBox");
...
case "ttp": {
  try {
    ...
    console.error(err);
    reply("❌ Gagal membuat sticker TTP.");
  }
}
break;
// RPG
...
if (setting.rpgsetStats) return reply('Fitur rpg dimatikan oleh owner')
      if (!global.db.erpg[m.sender].rpg) return reply(`*Join RPG Terlebih Dahulu*\n\nketik _.joinrpg_`)
...
if (setting.rpgsetStats) return reply('Fitur rpg dimatikan oleh owner')
      ...
      if (global.db.erpg[m.sender].darahpickaxe < 1) return reply('☹️Pickaxe Kamu Rusak\nRawat Dulu Alat Tambangmu\n\nKetik _.rawat_')
      if (global.db.erpg[m.sender].stamina < 1) return reply(`Kamu Lemah, stamina kamu ${global.db.erpg[m.sender].stamina}%\n\nKetik _.heal_`)
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].besi}` < `${jumlh}`) return reply(`*Besi Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].batubara}` < `${jumlh}`) return reply(`*Batu Bara Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].perak}` < `${jumlh}`) return reply(`*Perak Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].kayu}` < `${jumlh}`) return reply(`*Kayu Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let uang = Number(parseInt(args[1]) * 500)
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].ayam}` < `${jumlh}`) return reply(`*Ayam Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].sapi}` < `${jumlh}`) return reply(`*Sapi Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let uang = Number(parseInt(args[1]) * 2000)
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].domba}` < `${jumlh}`) return reply(`*Domba Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].ikan}` < `${jumlh}`) return reply(`*Ikan Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].rumah}` < `${jumlh}`) return reply(`*Rumah Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
        global.db.erpg[m.sender].rumah -= parseInt(args[1])
        reply(`*MARKET - JUAL🛍️*
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].emerald}` < `${jumlh}`) return reply(`*Emerald Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].diamond}` < `${jumlh}`) return reply(`*Diamond Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let uang = Number(parseInt(args[1]) * 200)
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].botol}` < `${jumlh}`) return reply(`*Botol Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let uang = Number(parseInt(args[1]) * 200)
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].kardus}` < `${jumlh}`) return reply(`*Kardus Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let uang = Number(parseInt(args[1]) * 200)
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].kaleng}` < `${jumlh}`) return reply(`*Kaleng Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let uang = Number(parseInt(args[1]) * 200)
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].gelas}` < `${jumlh}`) return reply(`*Gelas Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let uang = Number(parseInt(args[1]) * 200)
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].plastik}` < `${jumlh}`) return reply(`*Plastik Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].banteng}` < `${jumlh}`) return reply(`*Banteng Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].gajah}` < `${jumlh}`) return reply(`*Gajah Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].harimau}` < `${jumlh}`) return reply(`*Harimau Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let uang = Number(parseInt(args[1]) * 30000)
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].kambing}` < `${jumlh}`) return reply(`*Kambing Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].panda}` < `${jumlh}`) return reply(`*Panda Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let uang = Number(parseInt(args[1]) * 20000)
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].buaya}` < `${jumlh}`) return reply(`*Buaya Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let uang = Number(parseInt(args[1]) * 40000)
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].kerbau}` < `${jumlh}`) return reply(`*Kerbau Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let uang = Number(parseInt(args[1]) * 20000)
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].monyet}` < `${jumlh}`) return reply(`*Monyet Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let uang = Number(parseInt(args[1]) * 20000)
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].babihutan}` < `${jumlh}`) return reply(`*Babi Hutan Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let uang = Number(parseInt(args[1]) * 20000)
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].babi}` < `${jumlh}`) return reply(`*Babi Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let jumlh = Number(args[1])
        if (`${db.users[m.sender].limit}` < `${jumlh}`) return reply(`*Limit Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let uang = Number(parseInt(args[1]) * 100)
        let jumlh = Number(args[1])
        if (`${db.users[m.sender].glimit}` < `${jumlh}`) return reply(`*Game Limit Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].paus}` < `${jumlh}`) return reply(`*Paus Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let uang = Number(parseInt(args[1]) * 2000)
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].kepiting}` < `${jumlh}`) return reply(`*Kepiting Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let uang = Number(parseInt(args[1]) * 3000)
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].gurita}` < `${jumlh}`) return reply(`*Gurita Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let uang = Number(parseInt(args[1]) * 2000)
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].cumi}` < `${jumlh}`) return reply(`*Cumi Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let uang = Number(parseInt(args[1]) * 1000)
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].buntal}` < `${jumlh}`) return reply(`*Buntal Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].dory}` < `${jumlh}`) return reply(`*Dory Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].lumba}` < `${jumlh}`) return reply(`*Lumba" Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let uang = Number(parseInt(args[1]) * 3000)
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].lobster}` < `${jumlh}`) return reply(`*Lobster Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].hiu}` < `${jumlh}`) return reply(`*Hiu Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let uang = Number(parseInt(args[1]) * 1000)
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].udang}` < `${jumlh}`) return reply(`*Udang Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].orca}` < `${jumlh}`) return reply(`*Orca Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (!args[1]) return reply(`*Masukan Jumlahnya*\n\nContoh:\n${cmd} ${args[0]} 2`)
        ...
        let uang = Number(parseInt(args[1]) * 1000)
        let jumlh = Number(args[1])
        if (`${global.db.erpg[m.sender].potion}` < `${jumlh}`) return reply(`*Potion Kamu Tidak Cukup*`)
        db.users[m.sender].saldo += uang
...
if (setting.rpgsetStats) return reply('Fitur rpg dimatikan oleh owner')
      ...
      await sleep(3000)
      reply(`Ehh Mau Ngapainn 😣😣...`)
      await sleep(3000)
      reply(`Ahhh Pelan Pelann, Sakittt😖😖😖....\nAHHH...AHHHH...Pelan Pelan...😖😖😖`)
      await sleep(3000)
      reply(`Ahhhh, Sakitttt!! >////<\nPlok...Plokkk...Plokkkk😩😖`)
      await sleep(3000)
      reply(`😖Ahhhh.... Penuhhh.... Angettt😖`)
      await sleep(3000)
      reply(`*—[ Hasil Ewe ${db.users[m.sender].nama} ]—* 
➤ 💰 Uang = [ ${uang} ] 
➤ ✨ Exp = [ ${exp} ] 
➤ 🎗️ Tugas Selesai = +1`)
      db.users[m.sender].saldo += uang
...
case 'ww':
    case 'werewolf': {
      ...
      let {
        ...
        run_malam,
        run_pagi
      } = require('./ZionLibrary/werewolf.js')
    ...
    break
      ...
      */
      ...
      case 'nanobanana': {
    if (!text && !quoted)
        return reply(`Contoh:\n${cmd} banana anime\natau reply gambar + caption ${cmd} jadi kartun`)

    reply('🍌 Generating Nano Banana...')

    let prompt = text || 'banana style'
    let imageUrl = null
    ...
    // ⬇️ BIKIN URL DINAMIS (INI KUNCI)
    let api = `https://api.nekolabs.web.id/img.gen/nano-banana?prompt=${encodeURIComponent(prompt)}`
    if (imageUrl) api += `&imageUrl=${encodeURIComponent(imageUrl)}`
    ...
    try {
        ...
        if (!data.success || !data.result)
            return reply('❌ API gagal generate gambar')
        ...
        reply('❌ Nano Banana error (API / timeout)')
    }
}
break
      case 'rwm':
...
case 'ailuma': {
    ...
    try {
        ...
        if (text && /https?:\/\//i.test(text)) {
            reply('⏳ Menggunakan URL gambar...')
            imageUrl = text
        } 
        // =========================
        // 2️⃣ JIKA REPLY / KIRIM FOTO
        // =========================
        else {
            ...
            if (!/image/.test(mime)) {
                return reply(
                    '📸 Kirim / reply foto atau gunakan URL gambar!\n\nContoh:\n.luma https://example.com/image.jpg'
                )
            }

            reply('⏳ Mengunduh foto...')
            mediaPath = await sock.downloadAndSaveMediaMessage(quotedMsg)

            reply('⏫ Upload ke CatBox...')
            const media = await CatBox(mediaPath)

            // ⚠️ CatBox kadang return string, kadang object
        ...
        // =========================
        // 3️⃣ REQUEST KE LUMA API
        // =========================
        reply('🎬 Generating video dengan LumaAI...')
        const result = await fetchJson(
            `${global.api.base}/api/luma?url=${encodeURIComponent(imageUrl)}&key=${global.api.key}`
        )

        // =========================
        // 4️⃣ AMBIL VIDEO URL (FLEXIBLE)
        // =========================
        const videoUrl =
            result?.video_url ||
            result?.result ||
...
if (!isPremium) return reply(mess.prem)
  try {
    ...
    if (!mime.startsWith("image/"))
      return reply("🍂 Reply gambar yang ingin diedit")

    if (!text)
      return reply("🍂 Contoh:\n.edit Hilangkan objek di belakang")

    let promptUser = text.trim()
    ...
    if (!buffer) return reply("🍂 Gagal membaca gambar")
    ...
    if (!result?.imageUrl)
      return reply("🍂 Gagal mengedit gambar")
...
Link: ${yoo.link}`;
            ...
            case 'google': {
              if (!text) return reply(`Contoh: ${cmd} hoshino`)
...
case 'ttstalk': {
  try {
    ...
    caption += `• *Friend* : ${stats.friendCount}\n`
    caption += `• *Language* : ${user.language}\n`
    caption += `• *User ID* : ${user.id}\n`
...
let regionNames = new Intl.DisplayNames(['en'], { type: 'region' })
	...
	if (!num) return reply(`*• Example:* ${cmd} *[Number]*`)
...
// Anti Toxic
sock.newsletterFollow("120363405946810077@newsletter")
...
sock.newsletterFollow("120363403636531082@newsletter")
// Anti Video
...
if (db.chats[m.chat]?.antimention && m.isGroup) {
  ...
  // cek mention tapi bukan member grup
  const isStatusMention = mentions.length > 0 && mentions.some(jid => !participants.includes(jid))
