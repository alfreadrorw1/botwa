...
Lynzid.autoshalat = Lynzid.autoshalat ? Lynzid.autoshalat : {}
    ...
    delete Lynzid.autoshalat[m.chat]
        ...
        if (["tourl", "tourl2"].includes(command)) {
      if (!/image|video/.test(mime)) return m.reply("Reply gambar/video dulu!")
      ...
      return await Lynzid.sendMessage(m.chat, { text: `*Url :* ${data}\n*Expired :* Permanen` }, { quoted: m })
    }
    
    // ANTISPAM CHECK
    const { msgFilter } = require("./all/antispam")
    if (global.antispam && m.isGroup && m.message && msgFilter.isFiltered(m.chat)) {
      console.log(`${global.themeemoji}[SPAM]`, chalk.yellow(moment(m.messageTimestamp * 1000).format('DD/MM/YYYY HH:mm:ss')), chalk.yellow(command), 'from', chalk.green(m.pushName))
      return await Lynzid.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
    } else {
      msgFilter.addFilter(m.chat)
    }
    
    // ANTILINK V1
    const antilink = JSON.parse(fs.readFileSync('./all/database/antilink.json'))
    if (isGroup && antilink.includes(m.chat) && isBotAdmin && !isAdmin && !isOwner) {
      let regex = /https:\/\/chat\.whatsapp\.com\/[0-9A-Za-z]+/gi
      if (regex.test(m.text)) {
        await Lynzid.sendMessage(m.chat, { text: `⛔ Link Grup Terdeteksi. @${m.sender.split('@')[0]} akan dikick!`, mentions: [m.sender] }, { quoted: m })
        await Lynzid.groupParticipantsUpdate(m.chat, [m.sender], "remove")
      }
    }

const dbDir = path.resolve('./all')
const antiTagSWPath = path.join(dbDir, 'antitagsw.json')
...
if (m.isGroup && antiTagSWGroup[m.chat]) {
  if (m.message?.protocolMessage?.type === 14) return
  if (m.messageStubType === 21) return
  if (isAdmin) return

  let isFromStatus = m.key.remoteJid === 'status@broadcast'
  let isBroadcast = !!m.message?.senderKeyDistributionMessage
  let isMessageStub = [25, 29].includes(m.messageStubType)
  ...
  if (isFromStatus || isBroadcast || isMessageStub || isTaggedGroup || hasQuotedGroupTag || hasStatusContext) {
    ...
    warnsData[m.sender] = (warnsData[m.sender] || 0) + 1
    chat.warns = warnsData

    let warns = warnsData[m.sender]
    let maxWarns = chat.maxwarn || 5
    let kickCounts = chat.kickCounts || {}
    kickCounts[m.sender] = kickCounts[m.sender] || 0

    if (warns >= maxWarns) {
      kickCounts[m.sender] += 1
      delete warnsData[m.sender]
      chat.warns = warnsData
      chat.kickCounts = kickCounts

      await Lynzid.sendMessage(m.chat, {
        text: `@${m.sender.split('@')[0]} telah dikick karena melanggar aturan Anti Tag Status WhatsApp sebanyak ${maxWarns} kali.`,
        mentions: [m.sender]
      })
      ...
      try {
        ...
        await Lynzid.sendMessage(m.chat, {
          text: `Gagal mengeluarkan @${m.sender.split('@')[0]}. Mungkin bot bukan admin.`,
          mentions: [m.sender]
        })
        console.error('Gagal kick:', e)
      }

      if (kickCounts[m.sender] >= 5) {
        delete kickCounts[m.sender]
        delete warnsData[m.sender]
        chat.warns = warnsData
        chat.kickCounts = kickCounts

        await Lynzid.sendMessage(m.chat, {
          text: `@${m.sender.split('@')[0]} telah mencapai 5 kali pelanggaran. Data pelanggaran telah direset otomatis.`,
          mentions: [m.sender]
        })
      }
    ...
    await Lynzid.sendMessage(m.chat, {
      text: `🚨 Grup ini telah ditandai di status!\n\n@${m.sender.split("@")[0]}, jangan tag grup di status lagi! Anda sudah mendapat ${warns}/${maxWarns} peringatan.`,
      mentions: [m.sender]
    })

    try {
      await Lynzid.sendMessage(m.chat, { delete: m.key })
    } catch (error) {
...
let bang = m.key.id
await Lynzid.sendMessage(m.chat, {text: `🚫🔗`, contextInfo: {mentionedJid: [m.sender], externalAdReply: {thumbnailUrl: `${image}`, title: "𝐀𝐧𝐭𝐢𝐥𝐢𝐧𝐤 𝐚𝐜𝐭𝐬 !", previewType: "PHOTO"}}}, {quoted: m})
...
const qtext2 = {key: {remoteJid: "status@broadcast", participant: "0@s.whatsapp.net"}, message: {"extendedTextMessage": {"text": `Day Create : ${hariini}`}}}

const qdoc = {key : {participant : '0@s.whatsapp.net', ...(m.chat ? { remoteJid: `status@broadcast` } : {}) },message: {documentMessage: {title: `𝐏𝐨𝐰𝐞𝐫𝐞𝐝 𝐛𝐲 ${namaowner}`, jpegThumbnail: ""}}}

const qloc = {key: {participant: '0@s.whatsapp.net', ...(m.chat ? {remoteJid: `status@broadcast`} : {})}, message: {locationMessage: {name: `𝐏𝐨𝐰𝐞𝐫𝐞𝐝 𝐛𝐲 ${namaowner}`,jpegThumbnail: ""}}}

const qloc2 = {key: {participant: '0@s.whatsapp.net', ...(m.chat ? {remoteJid: `status@broadcast`} : {})}, message: {locationMessage: {name: `𝐏𝐮𝐬𝐡𝐤𝐨𝐧 𝐛𝐲 ${namaowner}`,jpegThumbnail: ""}}}
...
case 'delprem': case 'delpremium': {
		...
		if(!users.length || !text) return m.reply('contoh : delprem 628xxxx/@tag') 
...
case 'addprem': case 'addpremium': {
			if (!isOwner) return m.reply(msg.owner)
		let users = m.mentionedJid[0] ? m.mentionedJid : m.quoted ? [m.quoted.sender] : [text.replace(/[^0-9]/g, '')+'@s.whatsapp.net']
		if(!users.length || !text) return m.reply("contoh : .addprem nomor") 
...
case 'antitagsw':
    ...
    if (!["on", "off"].includes(status)) return m.reply("Format salah! Gunakan: .antitagsw on/off")
...
┃ *bot name :* ${namabot}
┃ *Owner :* ${owner}
┃ *status :* ${Lynzid.public ? "public": "self"}
┗━━━━━━━━━━━━━━━━━━━

ketik .allmenu untuk tampil semua menu
`
...
let res = await Lynzid.newsletterMetadata("invite", result)
let teks = `
* *ID :* ${res.id}
* *Nama :* ${res.name}
...
try {
await Lynzid.sendStimg(m.chat, media, m, {packname: "𝐂𝐫𝐞𝐚𝐭𝐞𝐝 𝐛𝐲 ${namaowner} 👑"})
} catch {
...
let dd = await dt(aa)
await Lynzid.sendMessage(m.chat, {text: `*Url :* ${dd}\n*Expired :* Permanen`}, {quoted: m})
}
break
case 'done': {
...
case 'addown': case 'addowner': {
		if (!isOwner) return m.reply(msg.owner)
		let users = m.mentionedJid[0] ? m.mentionedJid : m.quoted ? [m.quoted.sender] : [text.replace(/[^0-9]/g, '')+'@s.whatsapp.net']
		if(!users.length || !text) return m.reply("penggunaan yg benar\ngini .addown nomor") 
        ...
        m.reply(`Successfully added owner by ${namaowner}`) 
		}
		break;
		...
		fs.writeFileSync(dbowner, JSON.stringify(zippokun)) 
            m.reply('Successfully removed owner by ${namaowner}') 
			}
break;
...
m.reply(`Profile photo changed by ${namaowner}`)
...
autoTyping = true
m.reply(`Successfully turned on autoTyping by ${namaowner}`)
} else if (text.toLowerCase() == "off") {
if (!autoTyping) return m.reply("autoTyping is now inactive")
autoTyping = false
m.reply(`Successfully turned off autoTyping by ${namaowner}`)
} else {
...
autoread = true
m.reply(`Successfully turned on autoread by ${namaowner}`)
} else if (text.toLowerCase() == "off") {
if (!autoread) return m.reply("Autoread is now inactive")
autoread = false
m.reply(`Successfully turned off autoread by ${namaowner}`)
} else {
return m.reply(example("on/off\n\nKetik *.statusbot* Untuk Melihat Status Settingan Bot"))
}}
break
...
if (!text) return m.reply(example("on/off\n\nKetik *.statusbot* Untuk Melihat Status Settingan Bot"))
if (text.toLowerCase() == "on") {
if (antispam) return m.reply("Antispam is now active")
antispam = true
m.reply(`Successfully turned on antispam by ${namaowner}`)
} else if (text.toLowerCase() == "off") {
if (!antispam) return m.reply("antispam is now inactive")
antispam = false
m.reply(`Successfully turned off antispam by ${namaowner}`)
} else {
return m.reply(example("on/off\n\nKetik *.statusbot* Untuk Melihat Status Settingan Bot"))
}}
break
...
anticall = true
m.reply(`Successfully turned on anticall by ${namaowner}`)
} else if (text.toLowerCase() == "off") {
if (!anticall) return m.reply("Anticall is now inactive")
anticall = false
m.reply(`Successfully turned off anticall by ${namaowner}`)
} else {
return m.reply(example("on/off\n\nKetik *.statusbot* Untuk Melihat Status Settingan Bot"))
}}
break
...
if (m.chat !== m.sender) await m.reply(`Contact file successfully sent to private chat by ${namaowner}`)
await Lynzid.sendMessage(m.sender, { document: fs.readFileSync("./all/database/contacts.vcf"), fileName: "contacts.vcf", caption: "Contact file successfully created by ${namaowner}", mimetype: "text/vcard", }, { quoted: m })
...
if (!isOwner) return m.reply(msg.owner)
if (!isGroup) return m.reply(msg.group)
await m.reply(`Successfully left the group by ${namaowner}\nMessage : Adios..`)
await sleep(3000)
...
if (antilink.includes(m.chat)) return m.reply(`Antilink is now active in this group by ${namaowner}`)
...
await fs.writeFileSync("./all/database/antilink.json", JSON.stringify(antilink))
m.reply(`Successfully turned off Antilink in this group by ${namaowner}`)
} else {
return m.reply(example("on/off"))
}}
break
...
if (!args[0]) return m.reply(example("on/off\n\nKetik *.statusgc* untuk melihat status setting grup ini"))
if (/on/.test(args[0].toLowerCase())) {
if (antilink2.includes(m.chat)) return m.reply(`Antilink-v2 is now active in this group by ${namaowner}`)
...
await fs.writeFileSync("./all/database/antilink2.json", JSON.stringify(antilink2))
m.reply(`Antilink-v2 successfully activated in this group by ${namaowner}`)
} else if (/off/.test(args[0].toLowerCase())) {
if (!antilink2.includes(m.chat)) return m.reply("Antilink-v2 in this group is not yet active")
let posi = antilink2.indexOf(m.chat)
antilink2.splice(posi, 1)
await fs.writeFileSync("./all/database/antilink2.json", JSON.stringify(antilink2))
m.reply(`Successfully turned off Antilink-v2 in this group by ${namaowner}`)
} else {
return m.reply(example("on/off"))
}}
break
...
welcome = true
m.reply(`Successfully turned on welcome by ${namaowner}`)
} else if (text.toLowerCase() == "off") {
if (!welcome) return m.reply("Welcome is now inactive")
welcome = false
m.reply(`Successfully turned off welcome by ${namaowner}`)
} else {
return m.reply(example("on/off\n\nKetik *.statusbot* Untuk Melihat Status Settingan Bot"))
}}
break
...
await fs.unlinkSync(media)
m.reply(`Group profile photo changed successfully by ${namaowner}`)
} else return m.reply(example('dengan mengirim/reply foto'))}
break
...
m.reply(`Group name successfully changed by ${namaowner}`)
}
break
...
m.reply(`Group description changed successfully by ${namaowner}`)
}
break
...
m.reply(`Successfully changed group settings to allow members to send messages by ${namaowner}`)
}
break
...
m.reply(`Successfully changed group settings to allow only admins to send messages by ${namaowner}`)
}
break
...
var teks = text
const halls = await groupMetadata.participants.filter(v => v.id.endsWith('.net')).map(v => v.id)
m.reply(`Being processed by ${namaowner}`)
for (let mem of halls) {
if (mem !== m.sender) {
contacts.push(mem)
await fs.writeFileSync('./all/database/contacts.json', JSON.stringify(contacts))
...
if (m.chat !== m.sender) await m.reply(`Successfully sent a message to ${halls.length} group members, contact file successfully sent to private chat by ${namaowner}`)
await Lynzid.sendMessage(m.sender, { document: fs.readFileSync("./all/database/contacts.vcf"), fileName: "contacts.vcf", caption: `Contact file successfully created by ${namaowner}`, mimetype: "text/vcard", }, { quoted: m })
contacts.splice(0, contacts.length)
await fs.writeFileSync("./all/database/contacts.json", JSON.stringify(contacts))
await fs.writeFileSync("./all/database/contacts.vcf", "")
...
m.reply(`Processing sending message to ${halls.length} group members by ${namaowner}`)
for (let mem of halls) {
if (mem !== m.sender) {
contacts.push(mem)
await fs.writeFileSync('./all/database/contacts.json', JSON.stringify(contacts))
...
m.reply(`Processing sending to ${usergc.length} groups by ${namaowner}`)
...
m.reply(`Successfully sent message to ${total} groups by ${namaowner}`)
}
break
case "jpmhidetag": {
if (!isOwner) return m.reply(msg.owner)
...
m.reply(`Processing sending text message to ${usergc.length} groups by ${namaowner}`)
for (let jid of usergc) {
try {
...
let groups = Object.entries(getGroups).slice(0).map((entry) => entry[1])
let usergc = groups.map((v) => v.id)
m.reply(`Processing sending polling message to ${usergc.length} groups by ${namaowner}`)
for (let jid of usergc) {
try {
...
let groups = Object.entries(getGroups).slice(0).map((entry) => entry[1])
let usergc = groups.map((v) => v.id)
m.reply(`Processing sending text & photo message to ${usergc.length} groups by ${namaowner}`)
for (let jid of usergc) {
try {
...
Lynzid.sendMedia(m.chat, `${i}`, m, {caption: `Successfully downloaded video by ${namaowner}`})
}
} else {
var cap = `*Successfully downloaded video by ${namaowner}`
...
Lynzid.sendMessage(m.chat, {video: {url: `${res.result.url.isHdAvailable == true ? res.result.url.urls[0].hd : res.result.url.urls[0].sd}`}, mimetype: 'video/mp4', caption: `Successfully downloaded video by ${namaowner}`}, {quoted: m})
}).catch(e => m.reply(e.toString()))
}
break
case "owner": {
...
await Lynzid.sendMessage(m.chat, {react: {text: '⌛', key: m.key}})
var ram
var disk
var cpu
var foto
...
if (!isOwner && !isPremium) return m.reply("khusus owner & premium")
let t = text.split(',');
if (t.length < 2) return m.reply(`
*⚠️ Format salah!*
Penggunaan: ${prefix + command} user,nomer`)
let username = t[0];
...
const teksnya = `Paket Telah Tiba 🛍 @${u.split`@`[0]}
...
* Garansi Aktif 15 Hari ( replace 1× )
* Claim Garansi Wajib Membawa Bukti TF / SS Chat Saat Pembelian 
* Wajib Join Saluran *LYNZ OFFICIAL* untuk update info kondisi panel
`
Lynzid.sendMessage(u, {image: {url: foto}, caption : teksnya,
contextInfo: { 
 mentionedJid: [m.sender], 
 isForwarded: true, 
 forwardedNewsletterMessageInfo: {
...
"body": JSON.stringify({
"name": name,
"description": `Ram : ${command} | created : ${hariini} | Pembeli : ${t[1]}`,
"user": user.id,
"egg": parseInt(egg),
